<template>
	<view class="s-field" :class="{'u-border-top': borderTop, 'u-border-bottom': borderBottom }">
		<view class="s-field-inner" :class="[type == 'textarea' ? 's-textarea-inner' : '', 's-label-postion-' + labelPosition]">
			<view class="s-label" :class="[required ? 's-required' : '']" :style="{
				justifyContent: justifyContent, 
				flex: labelPosition == 'left' ? `0 0 ${labelWidth}rpx` : '1'
			}">
				<view class="s-icon-wrap" v-if="icon">
					<u-icon size="32" :custom-style="iconStyle" :name="icon" :color="iconColor" class="s-icon"></u-icon>
				</view>
				<slot name="icon"></slot>
				<text class="s-label-text" :class="[this.$slots.icon || icon ? 's-label-left-gap' : '']">{{ label }}</text>
			</view>
			<view class="fild-body">
				<view class="u-flex-1 u-flex" :style="[inputWrapStyle]">
					<textarea v-if="type == 'textarea'" class="u-flex-1 s-textarea-class" :style="[fieldStyle]" :value="value"
					...
					<input
						...
						:style="[fieldStyle]"
						:type="type"
						class="u-flex-1 s-field__input-wrap"
						:value="value"
				...
				<u-icon :size="clearSize" v-if="clearable && value != '' && focused" name="close-circle-fill" color="#c0c4cc" class="s-clear-icon" @click="onClear"/>
				<view class="s-button-wrap"><slot name="right" /></view>
				<u-icon v-if="rightIcon" @click="rightIconClick" :name="rightIcon" color="#c0c4cc" :style="[rightIconStyle]" size="26" class="s-arror-right" />
			</view>
		</view>
		<view v-if="errorMessage !== false && errorMessage != ''" class="s-error-message" :style="{
 ...
 * @example <s-field v-model="mobile" label="手机号" required :error-message="errorMessage"></s-field>
 */
export default {
	name:"s-field",
	props: {
