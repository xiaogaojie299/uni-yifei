...
import {getRegionRangeStatistics,getRegionTodayRankStatistics,getRegionYesterdayRangeStatistics} from "@/utils/api"
...
export default {
  ...
  created() {
    this.areaList = JSON.parse(uni.getStorageSync("provinceCity"));
  },
  onReachBottom(){
    // 页面触底
    this.pageNo++;
    this.subsectionList=[{name:"测试"}]
     switch(this.index){
          case 0: this.getRanktotal(getRegionTodayRankStatistics)
          break;
          case 1:this.getRanktotal(getRegionYesterdayRangeStatistics)
          break;
          default:
            this.getRanktotal(getRegionRangeStatistics)
        }
  },
  methods: {
    confirm(e) {
      this.selectArea = e[e.length - 1];
      this.getRanktotal(getRegionTodayRankStatistics);  //默认显示今日排行
    },
    init(){
      this.pageNo=1;
    },
