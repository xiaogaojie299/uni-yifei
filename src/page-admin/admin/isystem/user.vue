      ...
      <view class="hspList" v-for="(item, index) in list" :key="index">
        <view class="hspList-top my-box">
          <view class="">
            ...
            <view
              class="status"
              :class="item.status == 1 ? 'open-color' : 'close-color'"
            >
              {{ item.status == 1 ? "启用" : "禁用" }}
            </view>
          </view>
        ...
        <view class="hspList-bottom my-box">
          <view class="flex">
            <view
              @tap="go_edit(item)"
              class="btn ml-20 mr-20 compile-btn flex-ver-center"
            >
              编辑
            </view>
            <view
              @tap="goDetail(item.id, item.status)"
              class="btn compile-btn flex-ver-center"
            >
              {{ item.status == 1 ? "禁用" : "启用" }}
            </view>

            <view @tap="del(item.id)" class="btn ml-20 del-btn flex-ver-center">
              删除
            </view>
          </view>
        </view>
...
export default {
  ...
  methods: {
    ...
    go_edit(item) {
      ...
      uni.navigateTo({
        url:
          "/page-admin/admin/isystem/edit-user" +
          "?form=" +
          JSON.stringify(item),
      });
    },
